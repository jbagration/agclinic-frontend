...
export const Specialist = ({ specialist, cityId, apiUrl }: { specialist: ISpecialists; cityId: number; apiUrl: string; }) => {
        ...
        ?   <SModal
            ...
            >
                ...
                >
                    ...
                    {imgArr ?
                        <img
                            src={`${apiUrl}/public/uploads/images/${imgArr[pageDoc]}?height=930`}
                            alt='document'
                            width={658}
                            height={930}
                        /> : null
    ...
    <Link href="/specialists" passHref>
      <Back />
    </Link>
  </div>
                <SH1>
            ...
            <SBGBlock>
                <SSpecialistContent>
                    <img
                        src={`${apiUrl}/public/uploads/images/${specialist?.main_img}?height=684`}
                        alt='photo'
                        height={684}
                        width={503}
                            ...
                            <SCertificate>
                                {specialist.certificate.filter((item) => item.status === 0).map(({id, url}) => 
                                    (Array.isArray(url) ?
                                        <img
                                            key={id}
                                            src={`${apiUrl}/public/uploads/images/${url[0]}?height=300`}
                                            alt='certificate'
                                            height={300}
                                            width={420}
                                            style={{'height': 'auto'}}
...
{
    activeTab === 3 && specialist && specialist?.beforeAfter 
    ?
    <SPaddingWrap>
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
            {specialist.beforeAfter.map(({ before_img, title }, i) => {
                return (
                    <div key={i} style={{ margin: '10px', textAlign: 'center', maxWidth: '600px' }}>
                        <div style={{ height: window.innerWidth <= 1024 ? '250px' : '500px', overflow: 'hidden' }}>
                            <img
                                src={`${apiUrl}/public/uploads/images/${before_img}`}
                                alt={`Before`}
                                style={{ maxWidth: '100%', height: '100%', objectFit: 'cover', marginBottom: '20px' }}
                            />
                        </div>
                        <p style={{ color: 'black', marginTop: '20px', marginBottom: '20px' }}>{title}</p>
                    </div>
                );
            })}
        </div>
